...
from d2l import torch as d2l
from transforms import FlattenTrans
import matplotlib
...
def evaluate_accuracy(net, data_iter):
    if isinstance(net, torch.nn.Module):
        net.eval()
    metric = Accumulator(2)  # Sum of correct predictions and total number of predictions
    with torch.no_grad():
...
print('initial accuracy', evaluate_accuracy(net, test_iter))
...
def predict_ch3(net, test_iter, n=6):
    ...
    preds = get_fashion_minst_labels(y_hat.argmax(axis=1))
    titles = [true + '\n' + pred for true, pred in zip(trues, preds)]
    print(titles[0:n])
    show_images(X[0:n].reshape(-1, 28, 28), 2, n, titles=titles[0:n])
    plt.show()
    return
